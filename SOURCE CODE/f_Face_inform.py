...
from gender_detection import f_my_genderimg
...
gender_detector =  f_my_genderimg.Gender_Model()
...
def get_face_info(im):
    ...
    if len(boxes_face)!=0:
        for box_face in boxes_face:
            ...
            # -------------------------------------- gender_detection ---------------------------------------
            face_features["gender"] = gender_detector.predict_gender(im,face_image)
            ...
            # -------------------------------------- out ---------------------------------------       
            face_features["Caption"]


            a=face_features["name"]
            #print(a)

            b=face_features["emotion"]
            #print(b)
            c='angry'
            e='disgust'
            f='fear'
            ...
            if b==c:
                d=f"{a} is angry, Cool him UP"
                face_features["Caption"]=d
            elif b==e:
                d=f"{a} is disgust, ..."
                face_features["Caption"]=d
                ...
                d=f"{a} is fear, Make the person Relax"
                face_features["Caption"]=d
                
            elif b==g:
                d=f"{a} is happy, Enjoying the Life"
                face_features["Caption"]=d

            elif b==h:
                d=f"{a} is neutral, Make the Person Happy"
                face_features["Caption"]=d
                
            elif b==i:
                d=f"{a} is sad, Make Person feel better"
                face_features["Caption"]=d
                
            elif b==j:
                d=f"{a} is Suprised, Check the reason for Suprise"
                face_features["Caption"]=d
    ...
    for data_face in out:
        ...
        if len(box) == 0:
            ...
            try:
                ...
                pass
            try:
                cv2.putText(img, "Caption: " +data_face["Caption"], (x0, y0-step-10*5), cv2.FONT_HERSHEY_SIMPLEX, fontSize, (0,255,0), thickness)
            except:
                pass
    return img
